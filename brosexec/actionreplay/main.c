...
extern unsigned char boot_dol[];
extern unsigned long boot_dol_size;
...
{
	int i;
	DOLHEADER *hdr = (DOLHEADER *) boot_dol;

	// Inspect text sections to see if what we found lies in here
	for (i = 0; i < MAXTEXTSECTION; i++) {
		if (hdr->textAddress[i] && hdr->textLength[i]) {
			_memcpy((void*)hdr->textAddress[i], ((unsigned char*)boot_dol) + hdr->textOffset[i], hdr->textLength[i]);
		}
	}
	...
	for (i = 0; i < MAXDATASECTION; i++) {
		if (hdr->dataAddress[i] && hdr->dataLength[i]) {
			_memcpy((void*)hdr->dataAddress[i], ((unsigned char*)boot_dol) + hdr->dataOffset[i], hdr->dataLength[i]);
		}
	}
